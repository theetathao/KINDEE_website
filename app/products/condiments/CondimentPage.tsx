  ...
  return (
    <main className="condimentPage">
      ...
      <section className="condimentListing">
        ...
        {filtered.length ? <div className={`condimentGrid ${viewMode === 'list' ? 'listView' : ''}`}>{filtered.slice(0, visible).map(({ product, index }) => <article className="condimentCard" key={product[0]}><div className="condimentCardImage"><img src={`${A}condiments/product-${String(index + 1).padStart(2, '0')}.png`} alt={product[0]} /></div><div className="condimentCardInfo"><h3>{product[0]}</h3><p>{product[1]}</p></div></article>)}</div> : <p className="noProducts">No condiments match your search.</p>}
