  ...
  return (
    <main>
      ...
      <section className="story taste">
        ...
        <div className="copy right"><span>02.</span><h2>It begins<br/>with a taste.</h2><p>Bringing the authentic flavors of Asia to the world, with care and quality in every spoonful.</p><a className="goldBtn" href="#products">Explore Now <Arrow /></a></div>
      </section>
      <section className="products" id="products">
        <div className="productIntro"><span>03.</span><h2>Explore Products</h2><p>Thai food culture is reflected in every flavor, inspired by authentic recipes and crafted for today&apos;s table.</p></div>
        ...
        <div className="categories">{['Sauces','Seasoning','Coconut Milk','Noodles','Rice','Specialties'].map((x,i)=><button className={i===0?'active':''} key={x}>{x}</button>)}</div>
      </section>
      ...
      <section className="catalog" id="catalog"><div className="copy left light"><span>06.</span><h2>Bring More to<br/>the Table.</h2><p>Explore Kin Dee’s complete range of Southeast Asian food products, from everyday essentials to authentic regional flavors.</p><a className="outlineBtn" href="#">Download Catalog</a></div><img src={`${A}catalog.png`} alt="Kin Dee product catalog and ingredients"/><em className="note discover">there’s always more to discover</em></section>
      <footer><div className="footerMain"><div><img className="footerLogo" src={`${A}logo.png`} alt="Kin Dee"/><div className="links"><div><a href="#about">About</a><a href="#products">Products</a><a href="#service">Services</a><a href="#">Recipes</a><a href="#">Contact</a></div><div><a href="#">Terms</a><a href="#">Privacy</a><a href="#">Cookies</a></div></div></div><div className="sealGrid">{Array.from({length:14},(_,i)=><span key={i}>✦</span>)}</div></div><div className="copyright"><span>©THE KIN DEE CO., LTD.</span><span>f&nbsp;&nbsp; 𝕏&nbsp;&nbsp; in&nbsp;&nbsp; ◎</span></div></footer>
    </main>
  );
}
